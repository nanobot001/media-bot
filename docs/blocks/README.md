...
| **[Block 5-5e](block-5-5e-durable-prewarm-runtime-ledger.md)** | Durable Prewarm Runtime Ledger | Planned | Persist cycle lifecycle and next-run evidence, recover safely across PM2 restarts, decouple scheduling from Plex sync, and prevent overlapping cycles. |
| **[Block 5-5f](block-5-5f-release-variant-availability-catalog.md)** | Release-Variant Availability Catalog | Planned | Preserve multiple exact variants per media scope and derive canonical unknown/A/B/C state while keeping MediaFlow separate from direct-play C. |
| **[Block 5-5g](block-5-5g-catalog-population-provider-truth.md)** | Catalog Population & Provider-Check Truth | Planned | Populate all bounded exact variants from Search/prewarm and distinguish successful uncached checks from provider errors or partial evidence. |
