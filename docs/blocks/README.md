...
| **[Block 5-5h](block-5-5h-unified-availability-projection.md)** | Unified Availability Projection | Implemented with MCP verification limitation | Discovery, Search, pre-warm APIs, CLI, MCP delegation, and UI expose one sanitized catalog-derived state with canonical domain and explicit TV scope; MCP 2.x currently blocks the legacy 1.x server test collection. |
