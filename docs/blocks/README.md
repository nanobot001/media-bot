...
| **[Block 5-5h](block-5-5h-unified-availability-projection.md)** | Unified Availability Projection | Implemented | Discovery, Search, pre-warm APIs, CLI, MCP delegation, and UI expose one sanitized catalog-derived state with canonical domain and explicit TV scope; later MCP 1.x compatibility repair restored the previously blocked server test collection. |
| **[Block 5-5c](block-5-5c-mediaflow-production-browser-adapter.md)** | MediaFlow Production Browser-Stream Adapter | Implemented with live canary pending | Route one exact eligible cached catalog variant through an opaque local MediaFlow session while direct play remains C, with a minimal cached-version selector and configuration-only rollback. |
| **[Block 5-5i](block-5-5i-mediaflow-resilient-segmented-streaming.md)** | MediaFlow Resilient Segmented Streaming | Implemented with limitations | Added capacity admission and reservations; segmented production, idle supervision, and sustained verification remain in the 5-5j successor program. |
| **[Block 5-5j](block-5-5j-mediaflow-comprehensive-browser-delivery.md)** | MediaFlow Comprehensive Browser Delivery | In Progress | Compatibility-first program for diagnosable routing, segmented playback, reliable seek/cleanup, release-class coverage, and safe alternate-version fallback. |
| **[Block 5-5j-1](block-5-5j-1-mediaflow-diagnostics-admission-evidence.md)** | MediaFlow Diagnostics And Admission Evidence | Implemented | Configurable structured diagnostics, versioned admission evidence, stale-decision projection, and dashboard/API explanations without changing delivery policy. |
| **[Block 5-5j-2](block-5-5j-2-mediaflow-segmented-producer-supervision.md)** | MediaFlow Segmented Producer And Supervision | Implemented | Opaque on-demand heavy-transcode HLS, byte-evidence startup/idle supervision, bounded segment metadata, pinned local HLS.js, and deterministic output proof at 120 seconds. |
...
| **[Block 5-8](block-5-8-mcp-python-sdk-v2-antigravity-compatibility.md)** | MCP Python SDK v2 And Antigravity Compatibility | Planned | Migrate the 23-tool stdio server to MCP 2.x while preserving tool schemas, JSON envelopes, permissions, and the direct project-interpreter Antigravity launcher. |
