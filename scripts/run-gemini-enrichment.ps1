...
$BatchSize = 10           # 10 items per batch to stay safely under 15 RPM
$DelaySeconds = 2        # Delay between batches (10 items * ~3s/item = 30s processing + 50s sleep = ~80s total window)
...
# Define the python database query script as a Here-String (avoids PowerShell quote/variable interpolation issues)
$countScript = @"
import sqlite3
import os
...
cursor = conn.cursor()
cursor.execute("SELECT COUNT(*) FROM library_items WHERE source = 'plex' AND (enrichment_json IS NULL OR enrichment_json = '' OR json_extract(enrichment_json, '$.source') IS NULL OR json_extract(enrichment_json, '$.source') != 'gemini')")
...
Write-Host "[$timestamp] Starting Gemini-backed enrichment batches" -ForegroundColor Green
...
while ($true) {
    ...
    if ($currentMissing -le 0) {
        Write-Host "No more items need enrichment." -ForegroundColor Green
        break
    }
...
Write-Host "          Gemini Enrichment Summary" -ForegroundColor Green
Write-Host "==================================================" -ForegroundColor Green
